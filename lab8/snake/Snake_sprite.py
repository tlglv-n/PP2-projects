...
class Food(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect.topleft = (random.randint(0, 19) * CELL_SIZE, random.randint(0, 19) * CELL_SIZE)
...
def show_go_screen():
    ...
    box.blit(font.render(f"Your last score is: {score}", True, BLACK), (125, 220))
    ...
    while waiting:
        ...
        for event in pygame.event.get(): 
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:   #starts the game when Enter/Return is pressed
                waiting = False
...
def init():
    ...
    file = open(f'level{level}.txt') #loads the current level's file
    for y in range(0, 21):
        for x in range(0, 21):
            ch = file.read(1) 
...
def main():
    ...
    while True:
        ...
        while pygame.sprite.spritecollide(food, walls, False) or pygame.sprite.spritecollide(food, body_sprite, False):
            food.rect.topleft = (random.randint(0, 19) * CELL_SIZE, random.randint(0, 19) * CELL_SIZE)

        #coliision check for snake's head and food
        if pygame.sprite.spritecollideany(head, food_sprite):
            b = Snake_body(snake_list[-1]) #creates new body section with passed parameeters of the last block in snake's body
            all_sprites.add(b)
            body_sprite.add(b)
            snake_list.append(b)
            food.rect.topleft = (random.randint(0, 19) * CELL_SIZE, random.randint(0, 19) * CELL_SIZE)
        ...
        if score - last_score >= 30 and score != last_score: 
            newlevel = True
            last_score = score
            level += 1
            speed += 2
            print("Level: ", level)
            print(len(os.listdir('./levels')))
            if level > len(os.listdir('./levels')): #checks if the level reached maximum
                final_stage = True
                t = pygame.Surface((400, 200))
                t.fill((255, 255, 255))
                t.blit(final1, (t.get_width()//2 - final1.get_width()//2, t.get_height()//2 - final1.get_height()//2))
                t.blit(final2, (t.get_width()//2 - final2.get_width()//2, t.get_height()//2 + 30 - final2.get_height()//2))
                SCREEN.blit(t, (0, 100))
                pygame.display.flip()
                time.sleep(2)
        ...
        if pygame.sprite.spritecollideany(head, walls) or pygame.sprite.spritecollideany(head, body_sprite):
            ...
            rect.blit(over, (rect.get_width()//2 - over.get_width()//2, rect.get_height()//2 - over.get_height()//2))
